...
- [ ] Run: `grep -rn "spec-compliance review\|code-quality review\|phase boundary" skills/` — expect zero hits (or only explicit negation sentences)
- [ ] Run: `grep -rln "spec-reviewer-prompt\|code-quality-reviewer-prompt" skills/` — expect zero hits
- [ ] Spot-check frontmatter (`name`, `description`) on every edited SKILL.md
- [ ] Build the FR/AC coverage matrix against this plan's phases (Story 1–4 in spec.md) — confirm every AC maps to a phase above
- [ ] Commit: `docs(026-streamline-plan-execute-review): add plan`
