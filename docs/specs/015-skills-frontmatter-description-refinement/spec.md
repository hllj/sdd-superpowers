...
An audit of all 19 SDD skills against Anthropic's official skill authoring best practices and the community frontmatter reference (shanraisshan/claude-code-best-practice) revealed four categories of defects:
...
3. **No `allowed-tools` on read-heavy skills** — The Claude Code skill specification supports an `allowed-tools` frontmatter field that pre-approves tool calls without a permission prompt during skill execution. Skills that always need `Bash` and `Read` — but have no project-level `settings.json` granting those tools — interrupt flow with repeated prompts.

4. **Two content bugs** — A broken integration table row in `sdd-execute` renders outside the table, hiding a sub-skill reference; and an inconsistent lowercase section heading in `systematic-debugging` ("## your human partner's Signals You're Doing It Wrong") breaks document style.
...
- Add `allowed-tools: Bash, Read` to every skill that always requires filesystem or shell access during execution
...
**Secondary user:** Developers — seeing `sdd-workflow` in the slash menu (confusion), encountering permission prompts mid-workflow (friction), and reading rendered skill content in the IDE (broken table, broken heading).

**Usage context:** Every SDD session. Descriptions are evaluated at startup; frontmatter fields affect both menu rendering and runtime tool permissions; content bugs surface whenever the affected skill body is read.
...
### Story 3: Reduced Permission Prompts on Read-Heavy Skills

**As** Claude executing a skill that always reads files or runs shell commands  
**I want** those tool calls pre-approved in the skill's frontmatter  
**So that** the workflow is not interrupted by permission prompts on every invocation

**Acceptance criteria:**

- [ ] **AC-3.1** The following skills have `allowed-tools: Bash, Read` added to frontmatter: `sdd-specify`, `sdd-plan`, `sdd-research`, `sdd-review`, `sdd-init`, `sdd-execute`, `sdd-tasks`, `sdd-brainstorm`, `using-git`, `verification-before-completion`, `systematic-debugging`, `requesting-code-review`
- [ ] **AC-3.2** Skills that dispatch agents but do not themselves run shell commands or read files directly (`sdd-workflow`, `subagent-driven-development`, `dispatching-parallel-agents`, `receiving-code-review`, `finishing-a-development-branch`, `sdd-spec-update`) do NOT receive `allowed-tools` (no spurious permissions)
- [ ] **AC-3.3** `test-driven-development` has `allowed-tools: Bash` added (it runs test commands but does not read project files itself)
- [ ] **AC-3.4** No existing skill body content is changed as part of this story

### Story 4: Fixed Integration Table in sdd-execute
...
- [ ] **AC-4.1** The `> **Note:**` block on line 88–89 of `sdd-execute/SKILL.md` is removed; its content (`sdd-superpowers:test-driven-development` is mandated for implementer subagents dispatched by `subagent-driven-development`) is moved into a prose note directly below the Integration table
- [ ] **AC-4.2** The Integration table in `sdd-execute/SKILL.md` renders with no rows appearing outside the table boundary

### Story 5: Consistent Heading Capitalisation in systematic-debugging
...
- [ ] **AC-5.1** The heading `## your human partner's Signals You're Doing It Wrong` is changed to `## Your Human Partner's Signals You're Doing It Wrong`
...
**FR-3 — allowed-tools frontmatter (derived from Story 3):**  
Skills that always invoke `Bash` or `Read` during execution MUST list those tools in `allowed-tools`. Skills that do not directly invoke those tools MUST NOT list them. The criterion is direct use in the skill's execution flow — not transitive use via sub-skills.

**FR-4 — Content bug repair (derived from Stories 4–5):**  
...
**NFR-2 — Description length:** No revised description exceeds 200 characters. The `description` field has a 1,024-character limit; revised descriptions should stay well under 200 to leave room for future `when_to_use` additions.

**NFR-3 — All 19 skills remain functional:** After changes, every skill body is syntactically valid Markdown with well-formed YAML frontmatter (validated by attempting to load each skill).

---

## Error Scenarios

**E-1 — `allowed-tools` field not recognised:** If the running Claude Code version does not support `allowed-tools` in skill frontmatter, permission prompts continue as before — no regression in functionality, only in friction reduction. This spec change is safe to deploy unconditionally.

**E-2 — `user-invocable: false` not recognised:** If the running version does not support `user-invocable`, `sdd-workflow` continues to appear in the slash menu — no regression in auto-invocation behaviour.

---

## Open Questions

None. The audit findings are complete and all changes are fully specified above.
