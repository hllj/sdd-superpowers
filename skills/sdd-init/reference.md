# SDD Init: Full Procedure Reference
...
**Important:** Once Step 5 begins, write all scaffold files in one uninterrupted sequence without pausing for user input between files.
...
| `memory/constitution.md` exists but `docs/specs/` does not | Skip Step 5.1 only; continue with Steps 5.2 and 5.3; warn user |
| `CLAUDE.md` exists but has no SDD content | Append SDD section after showing diff and getting approval |
| User skips git convention Q&A | Write no files for Step 5.4; warn; proceed with rest of scaffold |
