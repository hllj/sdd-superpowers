...
Gather critical technical context before committing to an implementation plan. Research agents investigate options so the plan is grounded in real-world constraints, not assumptions.

**Announce at start:** "I'm using the sdd-research skill to gather technical context for this feature."

**Core principle:** Research happens BEFORE planning. The goal is to surface facts that change decisions — library compatibility, performance characteristics, security implications, organizational constraints. Research output feeds directly into `sdd-plan`.

## When to Use This Skill

Run `sdd-research` when the feature involves:
- **Library choices** — multiple viable options exist and the trade-offs are non-obvious
...
- **Architectural risk** — the approach could have hidden complexity or failure modes
- **Organizational constraints** — company standards, approved tech stacks, deployment policies

Skip this skill for purely UI/UX features with no new dependencies or infrastructure.
