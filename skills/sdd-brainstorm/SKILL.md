...
Turn fuzzy ideas into validated designs through collaborative dialogue. Produces a `design.md` that feeds directly into `sdd-specify` as a fast-path input.

**Announce at start:** "I'm using the sdd-brainstorm skill to explore this idea before specifying."

**Core principle:** No premature specification. `sdd-specify` is for formalizing what you've already agreed to build. This skill is for figuring out what that is.

<HARD-GATE>
Do NOT invoke sdd-specify, sdd-plan, or any implementation skill until you have presented a design and the user has approved it. Do NOT write code. Do NOT make architectural decisions. This skill produces ONLY a design document.
...
Every project goes through this process. Even if the idea seems clear, brainstorming surfaces assumptions. The design can be short for simple ideas, but you MUST present it and get approval.
